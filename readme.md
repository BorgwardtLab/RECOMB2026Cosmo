...
Installation: `git clone` and `cd` into this repository. The package depends on the [bioverse](https://github.com/BorgwardtLab/bioverse) for data processing and evaluation pipelines, and on [cosmic-torch](https://github.com/BorgwardtLab/Cosmo) which contains the Cosmo layers. Install them with `pip install cosmic-torch` and `pip install bioverse`.
